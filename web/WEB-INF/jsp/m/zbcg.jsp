...
<div class="container ">
    ...
    <div class="base-bidding-head">
        <span class="curr item">招标公告</span>
        <span class="item">成交公告</span>
    </div>
...
//            itemIndex = $this.index();
        ...
        var dropload = $(selector).dropload({
            ...
            loadDownFn: function (me) {
                // 加载菜单一的数据
                if (itemIndex == '0') {
                    var currentPage = Math.ceil($(".clearfix.zbgg li").length / rows)+1;
                    var params=$('form.zbcg_search').serialize();
                    $.ajax({
                        type: 'GET',
                        url: '<%=basePath%>zbcg_data?currentPage=' + currentPage + '&rows=' + rows + '&flag=zbgg&'+params,
                        dataType: 'json',
...
//                                 数据加载完
                    ...
                    //跳转页面
                    window.location.href="zbcg_more?tab=cjgg";
                }
            }
        });
    });
