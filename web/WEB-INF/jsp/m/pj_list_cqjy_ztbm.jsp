...
<div class="js_dialog" id="loadingToast" style="display: none;">
    <div class="weui-mask"></div>
    <div class="weui-dialog">
        <div class="weui-dialog__bd" id="toast_div" style="height: 350px;overflow-y:scroll;"></div>
...
<div class="container ">
    ...
    <%--<div class="base-bidding-head">--%>
        <%--<span class="item">普通报名</span>--%>
        <%--<span class="curr item">专厅报名</span>--%>
    <%--</div>--%>
...
<footer class="base-footer-index">
    <div class="box">
        ...
        <a href="pj_list_cqjy?table=ztbm" class="active"><span class=" iconfont ">&#xe67a;</span><br>专厅报名</a>
...
//                } else
                if (itemIndex == '1') {
                    var currentPage = Math.ceil($(".clearfix.ztbm li").length / rows) + 1;
                    var params = $('form.zbcg_search').serialize();
                    $.ajax({
                        type: 'GET',
                        url: '<%=basePath%>pj_list_data?currentPage=' + currentPage + '&rows=' + rows + '&flag=ztbm&' + params,
                        dataType: 'json',
                        success: function (e) {
                            var result = '';
                            var data = e.data;
...
//                                    result += "<li><a onclick=\"bm_sub('" + data[i].isOpen + "','" + data[i].projectguid + "','ZT" + "','" + data[i].bmguid + "','','"+data[i].project_baoming_status+"')\"  href=\"javascript:;\" >"
                                    result += "<li><a onclick=\"bm_sub('" + data[i].IsOpen + "','" + data[i].ZhuanTingGuid + "','ZT" + "','" + data[i].BaoMingGuid + "','','" + data[i].project_baoming_status + "')\"  href=\"javascript:;\" >"
                                        + "<span class=''>" + data[i].ZhuanTingName + "&nbsp;&nbsp;&nbsp;&nbsp;专厅";
                                    ...
                                    if (data[i].project_baoming_status==0){
                                        ...
                                        result +="<br><span class='time'>报名截止时间: "+data[i].BaoMingEndDate+"</span>";
                                    }

                                    if (data[i].BaoMingGuid && data[i].BaoMingGuid != "") {
//                                    if (data[i].actVisible == 0) {
                                        result += "<span style='float:right;background: #50b450;color: #fff; font-size: 12px; padding: 2px 7px;'>"+data[i].baoMingStatusText+"</span>";
//                                        if (data[i].baoMingStatusText == '未报名') {
//                                            result += "<br><span class='time'>报名截止时间: " + data[i].BZJEndDate + "</span>";
//                                        } else {
//                                            result += "<span style='float:right;background: #50b450;color: #fff; font-size: 12px; padding: 2px 7px;'>已报名</span>";
//                                        }
//                                    } else {
//                                        result += "<span style='float:right;background: #999999;color: #fff; font-size: 12px; padding: 2px 7px;'>报名已结束</span>";
                                    }
...
//                                 数据加载完
                                ...
                                // 无数据
                                me.noData();
//                                $('.new-list.ztbm').append('<div class="dropload-down"><div class="dropload-noData">暂无数据</div></div>');
                            }
                            // 为了测试，延迟1秒加载
                            setTimeout(function () {
                                $('.clearfix.ztbm').append(result);
                                // 每次数据加载完，必须重置
                                me.resetload();
                            }, 100);
                        },
