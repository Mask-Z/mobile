...
<div class="detail-head">
    ...
    <c:if test="${news.infoid=='46206dba-aee5-49b3-ae4c-36c92b3044f3'}">
    
    <div class="mt10 clearfix">
 	<a href="http://720yun.com/t/60cjzsefvO8" target="_blank"><img src="image/hnc.jpg" width="100%" class="dsb"></a></div>
                                                    ...
                                                    <c:if test="${not empty news.infoid and not empty news.systemtype}">
                                                        <c:choose>
                                                            <c:when test="${not empty news.zt}">
                                                                <c:if test="${ (empty BaoMingGuid or BaoMingGuid eq '') and couldSign=='true'}">
                                                                    onclick="bm_sub('${news.isOpen}','${news.zt}','ZT','','','1')"
                                                                </c:if>
                                                                <c:if test="${not empty BaoMingGuid and couldSign=='true'}">
                                                                    onclick="bm_sub('${news.isOpen}','${news.zt}','ZT','${BaoMingGuid}','','3')"
                                                                </c:if>
                                                            </c:when>
                                                            <c:otherwise>
                                                                <c:if test="${news.ispllr=='1' and empty BaoMingGuid and couldSign=='true'}"> href="pj_list_cqjy?ProjectRegGuid=${news.infoid}"</c:if>
                                                                <c:if test="${news.ispllr!='1' and empty BaoMingGuid and couldSign=='true' }"> onclick="bm_sub('${news.isOpen}','${news.projectguid}','${news.systemtype}','','','1')" </c:if>
